...
print(pluton_df.columns[:-1].values + "(S)")

# Создание DataFrame для стандартизированных данных с метками кластеров
pluton_scaled_df = pd.DataFrame(pluton_scaled, columns=pluton_df.columns[:-1].values + "(S)")
pluton_scaled_df['Cluster_Scaled'] = labels_scaled

join_df = pluton_scaled_df.join(pluton_df).join(
    pd.DataFrame([pluton_df["Cluster_Unscaled"][i] == pluton_scaled_df['Cluster_Scaled'][i] - 1 for i in range(
        len(pluton_scaled_df['Cluster_Scaled']))], columns=["Same"]))
...
join_df.to_csv("pluton_joined.csv", index=False)
